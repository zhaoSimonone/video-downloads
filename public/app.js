...
function platformName(platform) { return platform === 'channels' ? '微信视频号' : platform === 'douyin' ? '抖音' : platform === 'instagram' ? 'Instagram' : '未知平台'; }
function setPlatform(platform) { state.platform = platform; $$('.platform-tab').forEach(tab => tab.classList.toggle('active', tab.dataset.platform === platform)); $('#urlInput').placeholder = platform === 'channels' ? '粘贴微信视频号链接…' : platform === 'instagram' ? '粘贴 Instagram Reel 链接…' : '粘贴抖音链接…'; $('#inputHint').textContent = platform === 'channels' ? '例如：https://channels.weixin.qq.com/…' : platform === 'instagram' ? '例如：https://www.instagram.com/reel/…' : '例如：https://v.douyin.com/…'; }
...
function historyPlatformClass(platform) { return platform === 'channels' ? 'channels-logo' : platform === 'instagram' ? 'instagram-logo' : 'douyin-logo'; }
function historyPlatformIcon(platform) { return platform === 'channels' ? '◉' : platform === 'instagram' ? '◎' : '♪'; }
...
async function pollInstagramCapture(captureId) {
  for (let attempt = 0; attempt < 240; attempt += 1) {
    await new Promise(resolve => setTimeout(resolve, 500));
    const response = await fetch(`/api/instagram/capture?id=${encodeURIComponent(captureId)}`);
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Instagram 捕获任务失败');
  ...
  throw new Error('Instagram 捕获超时，请重新打开并播放 Reel');
}
...
$('#captureButton').addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('/api/instagram/open', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ url: state.result.pageSource || state.result.source }) });
    const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Instagram 捕获启动失败');
    state.result = { ...state.result, captureId: data.id, message: data.message };
    renderResult(state.result);
    await pollInstagramCapture(data.id);
  } catch (error) {
...
$('#downloadButton').addEventListener('click', async () => {
  ...
  try {
    const filename = state.result.capturedId ? `instagram-reel-${new Date().toISOString().replace(/[-:TZ.]/g, '').slice(0, 14)}.mp4` : '';
    const downloadUrl = state.result.capturedId
      ? `/api/instagram/download?id=${encodeURIComponent(state.result.capturedId)}&filename=${encodeURIComponent(filename)}`
